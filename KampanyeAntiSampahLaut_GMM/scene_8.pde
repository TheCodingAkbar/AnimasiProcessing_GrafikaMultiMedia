...
float handWaveAngle = 0; // Sudut lambaian tangan
float handWaveSpeed = 0.1; // Kecepatan lambaian tangan
...
void scene_8() {
  ...
  // Update sudut lambaian tangan
  handWaveAngle += handWaveSpeed;
  
  // Gambar karakter dengan mulut yang bergerak sesuai audio
  int mouthState_8 = (int)((player_8.position() / 100) % 3); // Update status mulut berdasarkan posisi audio
  drawCharacter_8(200, 800, mouthState_8);
...
void drawCharacter_8(int x, int y, int mouthState) {
  // Kepala
  ...
  fill(255);
  if (mouthState == 0) {
    arc(x, y - 37.5, 45, 30, 0, PI); // Mulut tertutup
  } else if (mouthState == 1) {
    arc(x, y - 37.5, 45, 40, 0, PI); // Mulut setengah terbuka
  } else {
    arc(x, y - 37.5, 45, 50, 0, PI); // Mulut terbuka
  }
  ...
  ellipse(x + 90, y + 75, 30, 30); // Tangan kanan

  // Lengan dengan lambaian tangan
  fill(255, 224, 189); // Warna kulit
  pushMatrix();
  translate(x - 90, y);
  rotate(sin(handWaveAngle) * HALF_PI); // Rotasi tangan kiri untuk lambaian ke atas
  rect(0, 0, 30, 75, 15);
  ellipse(15, 75, 30, 30); // Tangan kiri
  popMatrix();
